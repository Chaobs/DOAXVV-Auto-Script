...
#检测程序运行状态的变量
STOPFLAG = False #此变量由主线程和子线程共同操作，并被同步锁锁死，后续版本可以考虑用队列实现
MAINTHREAD = True #True表示在主线程
lock = Lock() #全局线程锁，待优化
...
def keybound():
    ...
    #wait()
    #此部分在主函数里通过手动的无限循环实现不停监听STOPFLAG的情况
    return
    ...
    while True:
        ...
        try:
            ...
            while True:
                with lock:
                    if STOPFLAG: #停止
                        raise Stopflag #抛出自定义异常
        except Stopflag:
            if MAINTHREAD: #在主进程，退出程序
                _exit(0)
            else:
                unhook_all() #解除绑定
                with lock:
